...
Uses
  ...
  {$ELSE}
   {$if CompilerVersion > 21} // Delphi 2010 pra cima
    ...
    IdGlobal, IdCoderMIME, uDWConsts, uDWConstsData, IdHashMessageDigest;
   {$ELSE}
...
Begin
 {$IF Defined(ANDROID) OR Defined(IOS)} //Alterado para IOS Brito
  Result := TNetEncoding.Base64.Encode(S);//TIdEncoderMIME.EncodeString(S, IndyTextEncoding_utf8);
 {$ELSE}
  {$IFDEF FPC}
   Result := Base64Encode(S);
//   Result := EncodeStringBase64(S);
  {$ELSE}
    Result := TIdEncoderMIME.EncodeString(S, {$IFNDEF FPC}{$IF (CompilerVersion > 22)}
                                           IndyUTF8Encoding
                                          {$ELSE IF (CompilerVersion > 18)}
 ...
 {$ELSE}
  ...
  {$ELSE}
   {$IF (CompilerVersion > 18)}
    Result := TIdDecoderMIME.DecodeString(S, {$IFNDEF FPC}{$IF (CompilerVersion > 22)}
                                              IndyUTF8Encoding
                                             {$ELSE IF (CompilerVersion > 18)}
                                              IndyTextEncoding_utf8
                                             {$IFEND}
...
Begin
 ...
 {  SetLength(bytes, TMemoryStream(Value).Size * 2);
  ...
  If Length(Result) > 2 Then //Ajuste para retirar o Ponteiro de Nulo dos Hexas em Final de Arquivo.
   Begin
    If (Result[Length(Result) - FinalStrPos] = '0')     And
       (Result[Length(Result) - (1 + FinalStrPos)] = '0') Then
     Result := Copy(Result, InitStrPos, Length(Result) - (2 + FinalStrPos));
   End;
  {$IFEND}
  {$IFEND}
 Except
